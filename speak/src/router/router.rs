use crate::{
	crypto::encrypt::return_project_data,
	data::{
...
pub fn router() -> Arc<Router> {
	let core_router =
		<Router>::new()
				...
				t(|_ctx, input: Vec<String>| {
					...
					let data_to_save: ProjectData = ProjectData {
						...
						scene_data: SceneData {
							...
							scene_core: SceneCore { contents: _v },
			...
			.query("getProjectData", |t| {
				t(|_ctx, input: Vec<String>| {
					let _result = return_project_data(input[0].to_string());

					return _result;
				})
			})
			.merge("scene", scene::create_scene_router())
			.build()
			.arced();
